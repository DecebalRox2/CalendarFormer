...
with open("calendarformer.txt","w") as f3:
    ...
    while k < 213:
        ...
        f3.write("SUMMARY:" + (data2[k+1].split(":"))[1].split(" ")[0] + " " + (data2[k+1].split(":"))[1].split(" ")[1] + " " + (data2[k+5].split("\\"))[0].split(":")[1] + " " + "(" + (((data2[k+1].split(":"))[1].split(" ")[2]).upper()).strip("\n") + ")") # Adds summary
        f3.write("\nCLASS:PUBLIC\n") # Adds another parameter I have no clue about
        f3.write(data2[k+7] if (((data2[k+1].split(":"))[1].split(" ")[2]).upper()).strip("\n") != "FINAL" else "") # Adds repetiton rule if event is not a final
        f3.write("END:VEVENT")
        k += 10
    f3.write("\nEND:VCALENDAR")
